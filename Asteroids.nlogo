...
to-report s-find-a-path [ hunter-ship player] 
  ...
  ; add hunter patch in the open list
  set open lput hunter-ship open
  
  ; loop until we reach the target or the open list becomes empty
  if search-done? != true 
  [    
    ...
    [
      ...
      ask current-patch
      [   
        ; if any of the neighbors is the target stop the search process
        ifelse any? neighbors4 with [ (pxcor = [ pxcor ] of player) and (pycor = [pycor] of player)]
        [
        ...
        [
          ...
          [
            ; the neighbors to be explored should also not be the hunter or 
            ; target patches or already a part of the open list (unexplored patches list)
            if not member? self open and self != hunter-ship and self != player
            [
              set pcolor 0
    ...
    [
      ...
      user-message( "A path from the hunter to the target does not exist." )
      report []
    ]
  ]
  
  ; if a path is found (search completed) add the current patch 
  ; (node adjacent to the target) to the search path.
  set search-path lput current-patch search-path
  
  ; trace the search path from the current patch 
  ; all the way to the hunter ship using the parent patch
  ...
  ; add the target patch to the front of the search path
  set search-path fput player search-path
  
  ; reverse the search path so that it starts from a patch adjacent to the
  ; hunter patch and ends at the target patch
...
; make the hunter ship traverse (move through) the path all the way to the player
to s-move
  ask one-of ships with [label = "hunter"]
  [
    while [length current-path != 0]
    [
      s-go-to-next-patch-in-current-path
      pd
...
to s-go-to-next-patch-in-current-path
  ...
  if [label] of one-of ships != "hunter" and one-of players != "target"
  [
    ask patch-here
    [
      set pcolor black
...
to c-draw-boundaries ;; draw the boundary that confines the objects to screen
     ...
     [ set pcolor blue ]
     ask patches with [abs pycor = min-pycor ]
     [ set pcolor green]
end
...
to c-move-players ;; moves the player object on screen
 ask turtles
   [forward 1]
end

to c-color-refresh ;; checks boundary colour code
 ask patches[set pcolor  black]
 ask patches with [abs pxcor = max-pxcor]
   [ set pcolor blue ]
end
